...
export default function QuestionUpdation({ id, updateId }) {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${url}update-problem-admin`, {
            'question': { ...questions },
            id
        }).then((result) => {
            setQuestions({
    ...
    const getQuestion = async () => {
        axios.post(`${url}get-problem-admin`, { id }).then((result) => {
            let quest = result.data?.question
            delete quest.id
            setQuestions(quest)
            setLoading(false)
        }).catch((error) => {
    ...
    return (
        <div className="form2 my-8 question-addition p-4  shadow rounded mx-2 md:mx-0">
            <div className="flex p-2">
                <button className="bg-blue-400 text-white p-2 mr-12 rounded " onClick={handleBack}>Back</button>
                <h1 className='text-center text-2xl font-bold w-4/5'>

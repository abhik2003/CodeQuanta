...
def addquestions(data,problems):
...
def getAllQuestions(problems,page):
    qt=[]
    try:
        questions=list(problems.find({},{"statement":1,'description':1,'difficulty':1}).skip(page*10).limit(10))
        qt = [
            {
                "id": str(question["_id"]),
...
def getParticularQuestion(problems,id):
    ...
    try:
        obj_id = ObjectId(id)
        # print(obj_id)
        question=problems.find_one({"_id":obj_id},{"statement":1,'description':1,'difficulty':1})
        # print(question)
        if(question is None):
